...
#' @param group the group of genes you want to do GO enrichment, if it is NULL (default), then do GO enrichment for
#'              all the groups
#'
...
#'
#' @examples
get_enrich_plot <- function(st_res, group = NULL){
  if(!is.null(group)){
    ...
    gene_set <- list()
    for (i in 1:max(st_res$pl$data$group)) {
      group_rank <- which(st_res$pl$data$group == i)
      gene_set[[as.character(i)]] <- as.character(st_res$pl$data$vertex.names[group_rank])
    }
    enrich_res <- compareCluster(gene_set, fun = 'enrichGO', keyType = 'SYMBOL', OrgDb = org.Hs.eg.db, pvalueCutoff = 0.1)
    enrich_pl <- dotplot(enrich_res)
    res <- list(enrich_res = enrich_res, plot = enrich_pl)
  }
  return(res)
}
